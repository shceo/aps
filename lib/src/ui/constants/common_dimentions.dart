abstract final class CommonDimensions {
  // Dimensions
  static const double none = 0;
  static const double extraSmall = 2;
  static const double small = 4;
  static const double medium = 8;
  static const double mediumLarge = 12;
   static const double mediumLargeforWeek = 13;
  static const double large = 16;
  static const double dateSize = 15;
  static const double extraLarge = 32;
  static const double superLarge = 64;
  static const double multiLarge = 128;
  static const double megaLarge = 256;

  // ScreenSize
  static const double maxPhoneWidth = 500;
  static const double minDurationProgramWidth = 480;

  // Category assets sizes
  static const double categoryAssetsHeight = 154;

  // Padding
  static const double commonPadding = 12;

  // Another values
  static const double cToolbarHeight = 60;
  static const double checkBoxBorderWidth = 2;
  static const int minLines = 1;
  static const int maxLines = 2;
  static const int minSupportLines = 2;
  static const int maxSupportLines = 3;
  static const double rowCalendarHeight = 80;
  static const int daysCount = 30;
}
